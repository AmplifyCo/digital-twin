...
Restart=always
RestartSec=10
StandardOutput=append:/home/$USER/novabot/data/logs/cloudflared.log
StandardError=append:/home/$USER/novabot/data/logs/cloudflared.log
...
mkdir -p ~/novabot/data
cat > ~/novabot/data/cloudflare_tunnel.json << EOF
{
  "tunnel_id": "$TUNNEL_ID",
...
if [ -f ~/novabot/.env ]; then
    TELEGRAM_BOT_TOKEN=$(grep TELEGRAM_BOT_TOKEN ~/novabot/.env | cut -d '=' -f2 | tr -d ' "'"'"'')
